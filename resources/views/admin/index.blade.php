<x-admin.index-layout>
      {{-- catch messages  --}}
        ...
        @endif
      {{-- catch messages  --}}
    <form action="/admin/news-add" method="post" enctype="multipart/form-data" novalidate>
      @csrf
      <label for="news-title" class="form-label">Yangilik sarlavhasi</label>
      <input type="text" name="title" id="news-title" placeholder="Yangilik uchun sarlavha" class="form-control" required>
    
      <label for="editor" class="form-label mt-3">Yangilik Matni</label>
      <textarea name="description" id="editor" class="form-control bg-dark" cols="30" rows="10" placeholder="Yangilik matni" ></textarea>
